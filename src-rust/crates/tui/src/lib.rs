// jet-tui: Terminal UI using ratatui + crossterm for jet.
//
...
/// Live execution timeline state and helpers.
pub mod timeline;
...
mod tests {
    ...
    use jet_core::types::{ContentBlock, Role, ToolResultContent};
    ...
    fn test_render_app_keeps_logo_header_after_first_message() {
        ...
        let mut app = make_app();
        app.push_message(jet_core::types::Message::user("hello".to_string()));
    ...
    fn test_message_renderer_includes_tool_use_and_thinking_blocks() {
        let msg = jet_core::types::Message::assistant_blocks(vec![
    ...
    fn test_message_renderer_includes_tool_result_errors() {
        let msg = jet_core::types::Message::user_blocks(vec![ContentBlock::ToolResult {
    ...
    fn test_handle_status_event() {
        let mut app = make_app();
        app.handle_query_event(jet_query::QueryEvent::Status("working".to_string()));
    ...
    fn test_handle_error_event() {
        let mut app = make_app();
        app.is_streaming = true;
        app.handle_query_event(jet_query::QueryEvent::Error("oops".to_string()));
        assert!(!app.is_streaming);
        assert_eq!(app.messages.len(), 1);
    ...
    fn test_handle_tool_start_and_end() {
        let mut app = make_app();
        app.handle_query_event(jet_query::QueryEvent::ToolStart {
        ...
        app.handle_query_event(jet_query::QueryEvent::ToolEnd {
            tool_name: "Bash".to_string(),
            tool_id: "t1".to_string(),
    ...
    fn test_turn_complete_flushes_streaming_text() {
        ...
        app.handle_query_event(jet_query::QueryEvent::TurnComplete {
            turn: 1,
