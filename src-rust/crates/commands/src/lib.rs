...
fn parse_timeline_action(args: &str) -> Result<TimelineAction, String> {
    let parts = split_command_args(args);
    match parts.as_slice() {
        [] => Ok(TimelineAction::Toggle),
        [action] => match action.as_str() {
            "show" => Ok(TimelineAction::Show),
            "hide" => Ok(TimelineAction::Hide),
            "toggle" => Ok(TimelineAction::Toggle),
            "clear" => Ok(TimelineAction::Clear),
            _ => Err(format!(
                "Unknown timeline action '{}'. Usage: /timeline [show|hide|toggle|clear]",
                action
            )),
        },
        _ => Err("Usage: /timeline [show|hide|toggle|clear]".to_string()),
    }
}
...
fn command_category(name: &str) -> &'static str {
    match name {
        ...
        "cost" | "stats" | "timeline" | "usage" | "extra-usage" | "context" | "ctx-viz" => {
            "Usage & Cost"
        }
...
impl SlashCommand for TimelineCommand {
    ...
    fn description(&self) -> &str {
        "Control the live execution timeline panel"
    }
    fn help(&self) -> &str {
        "Usage: /timeline [show|hide|toggle|clear]\n\n\
         Controls the live execution timeline panel in the TUI.\n\
         Subcommands:\n\
           /timeline show    - open the timeline panel\n\
           /timeline hide    - close the timeline panel\n\
           /timeline toggle  - toggle the panel visibility (default)\n\
           /timeline clear   - clear retained timeline rows\n\n\
         TUI contract: intercept `/timeline <action>` before slash-command\n\
         execution and mutate timeline UI state directly. In non-TUI contexts,\n\
         this command falls back to a descriptive status message."
    }
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        CommandResult::Message(format!(
            "Timeline panel action requested: {}.\n\
             TUI interception contract: intercept `/timeline {}` before command execution and apply the UI state change directly.\n\
             No timeline panel is available in this non-TUI context.",
            action.as_str(),
            action.as_str(),
        ))
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_timeline_command_defaults_to_toggle_contract_message() {
        let mut ctx = make_ctx();
        let cmd = find_command("timeline").unwrap();
        let result = cmd.execute("", &mut ctx).await;
        let CommandResult::Message(msg) = result else {
            panic!("timeline should return a contract message outside the TUI");
        };

        assert!(msg.contains("Timeline panel action requested: toggle."));
        assert!(msg.contains("intercept `/timeline toggle`"));
    }

    #[tokio::test]
    async fn test_timeline_command_accepts_clear_subcommand() {
        let mut ctx = make_ctx();
        let cmd = find_command("timeline").unwrap();
        let result = cmd.execute("clear", &mut ctx).await;
        let CommandResult::Message(msg) = result else {
            panic!("timeline clear should return a contract message outside the TUI");
        };

        assert!(msg.contains("Timeline panel action requested: clear."));
        assert!(msg.contains("intercept `/timeline clear`"));
    }

    #[tokio::test]
    async fn test_timeline_command_rejects_unknown_subcommand() {
        let mut ctx = make_ctx();
        let cmd = find_command("timeline").unwrap();
        let result = cmd.execute("zoom", &mut ctx).await;
        let CommandResult::Error(msg) = result else {
            panic!("timeline should reject unknown actions");
        };

        assert_eq!(
            msg,
            "Unknown timeline action 'zoom'. Usage: /timeline [show|hide|toggle|clear]"
        );
    }
