...
impl BridgeConfig {
    ...
    /// - `jet_BRIDGE_URL` — overrides `server_url` and sets `enabled = true`
    /// - `jet_BRIDGE_TOKEN` / `CLAUDE_BRIDGE_OAUTH_TOKEN` — sets `session_token`
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(url) =
            std::env::var("jet_BRIDGE_URL").or_else(|_| std::env::var("CLAUDE_BRIDGE_BASE_URL"))
        {
            if !url.is_empty() {
        ...
        // Token override
        if let Ok(token) = std::env::var("jet_BRIDGE_TOKEN")
...
/// 1. `jet_BRIDGE_TOKEN` environment variable
...
/// match jet_bridge::start_bridge_session(None).await {
...
pub async fn start_bridge_session(
    ...
    let server_url = std::env::var("jet_BRIDGE_URL")
