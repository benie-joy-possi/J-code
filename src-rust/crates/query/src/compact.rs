...
use jet_core::types::{ContentBlock, Message, MessageContent, Role};
...
fn estimate_block_chars(block: &ContentBlock) -> usize {
    match block {
        ...
        ContentBlock::ToolResult { content, .. } => match content {
            jet_core::types::ToolResultContent::Text(t) => t.len(),
            jet_core::types::ToolResultContent::Blocks(blocks) => {
                ...
                match block {
                    ...
                    ContentBlock::ToolResult {
                        ...
                        let result_text = match content {
                            jet_core::types::ToolResultContent::Text(t) => {
                                t.as_str().to_string()
                            }
                            jet_core::types::ToolResultContent::Blocks(_) => {
...
pub struct CompactResult {
    ...
    pub messages: Vec<jet_core::types::Message>,
...
pub fn snip_compact(
    messages: Vec<jet_core::types::Message>,
    keep_n_newest: usize,
) -> (Vec<jet_core::types::Message>, u64) {
    let total = messages.len();
...
/// `reactiveCompact.ts`.
fn strip_images(messages: Vec<jet_core::types::Message>) -> Vec<jet_core::types::Message> {
...
/// `jet_core::feature_gates::is_feature_enabled("reactive_compact")` is true.
///
...
pub async fn reactive_compact(
...
) -> Result<CompactResult, jet_core::error::ClaudeError> {
    if cancel.is_cancelled() {
        return Err(jet_core::error::ClaudeError::Cancelled);
    }
    ...
    for path in recently_modified.iter().take(MAX_FILES * 3) {
        ...
        new_messages.push(jet_core::types::Message::user(text));
        injected += 1;
    }
...
pub async fn context_collapse(
    ...
    for msg in &messages {
        let role = match msg.role {
            jet_core::types::Role::User => "Human",
    ...
    let last_user = messages
        .iter()
        .rev()
        .find(|m| m.role == jet_core::types::Role::User)
        .cloned();
...
pub fn collapse_read_tool_results(
    ...
    use jet_core::types::{ContentBlock, MessageContent, ToolResultContent};
...
pub fn collapse_search_results(
    ...
    let mut result: Vec<jet_core::types::Message> = messages
        .into_iter()
        .rev()
        .map(|mut msg| {
            if let MessageContent::Blocks(ref mut blocks) = msg.content {
