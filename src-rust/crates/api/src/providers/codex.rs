...
//   ~/.jet/codex_tokens.json (`CodexTokens` struct).
//
...
//   so we use the `CODEX_MODELS` constant from `jet-core`.
...
use jet_core::oauth_config::{get_codex_tokens, save_codex_tokens, CodexTokens};
...
impl CodexProvider {
    ...
    async fn refresh_token(&self, refresh_token: &str) -> Result<String, ProviderError> {
        let body = json!({
            ...
            "client_id": jet_core::codex_oauth::CODEX_CLIENT_ID,
                                    ...
                                    match item.get("type").and_then(|value| value.as_str()) {
                                        ...
                                        Some("function_call") => {
                                            ...
                                            if open_blocks.insert(output_index) {
                                                yield Ok(StreamEvent::ContentBlockStart {
                                                    index: output_index,
                                                    content_block: jet_core::types::ContentBlock::ToolUse {
                                                        id: call_id,
                                                        name,
                                                        input: json!({}),
                                                    },
                                ...
                                if let Some(part) = json_val.get("part") {
                                    if matches!(part.get("type").and_then(|value| value.as_str()), Some("output_text") | Some("text")) {
                                        ...
                                        if open_blocks.insert(output_index) {
                                            yield Ok(StreamEvent::ContentBlockStart {
                                                index: output_index,
                                                content_block: jet_core::types::ContentBlock::Text {
                                                    text: String::new(),
                                                },
                                            });
                                        }
