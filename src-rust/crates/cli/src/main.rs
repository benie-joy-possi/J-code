...
//
// This is the main binary for JET. It:
...
#[command(
    name = "claude",
    version = APP_VERSION,
    about = "JET - AI-powered coding assistant",
    long_about = None,
)]
struct Cli {
    ...
    /// Grant JET access to an additional directory (can be repeated)
...
async fn main() -> anyhow::Result<()> {
    ...
    debug!(cwd = %cwd.display(), "Starting JET");
        ...
        if let Some(provider_id) = app.device_auth_pending.take() {
            ...
            match provider_id.as_str() {
                ...
                "anthropic" => {
                    ...
                    // JET does not have its own registered OAuth app with Anthropic.
...
fn subscription_label(subscription_type: Option<&str>) -> Option<String> {
    match subscription_type? {
        "enterprise" => Some("JET Enterprise Account".to_string()),
        "team" => Some("JET Team Account".to_string()),
        "max" => Some("JET Max Account".to_string()),
        "pro" => Some("JET Pro Account".to_string()),
