...
impl IdeKind {
    ...
    pub fn extension_install_command(&self) -> Option<String> {
        match self {
            Self::VSCode => Some("code --install-extension jet.jet".to_string()),
            Self::VSCodeInsiders => {
                Some("code-insiders --install-extension jet.jet".to_string())
            }
            Self::Cursor => Some("cursor --install-extension jet.jet".to_string()),
            Self::Windsurf => Some("windsurf --install-extension jet.jet".to_string()),
            Self::VSCodium => Some("codium --install-extension jet.jet".to_string()),
            _ => None,
        }
    }
}
