...
//
// History is persisted to `~/.jet/tip_history.json`.
...
static ALL_TIPS: Lazy<Vec<Tip>> = Lazy::new(|| {
    vec![
        ...
        Tip {
            id: "custom-commands",
            content: "Create skills by adding .md files to .jet/skills/ in your project or ~/.jet/skills/ for skills that work in any project",
            cooldown_sessions: 15,
        },
        Tip {
            id: "shift-tab",
...
impl TipHistory {
    /// Path to the persisted history file: `~/.jet/tip_history.json`.
    fn history_path() -> Option<std::path::PathBuf> {
        dirs::home_dir().map(|h| h.join(".jet").join("tip_history.json"))
    }

    /// Load history from `~/.jet/tip_history.json`.
    ...
    /// Persist history to `~/.jet/tip_history.json`.
