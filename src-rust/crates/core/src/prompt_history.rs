...
//   - Entries are written to `~/.jet/history.jsonl` via O_APPEND.
...
//     `~/.jet/pastes/<sha256-hex>` and referenced by hash.
...
/// Persist `text` to `~/.jet/pastes/<hash>`.  Fire-and-forget.
...
/// Read text from `~/.jet/pastes/<hash>`, returning `None` if missing.
...
/// Append `entry` to `~/.jet/history.jsonl`.
///
/// The call is fire-and-forget: it spawns a background Tokio task and returns
/// immediately.  If `jet_SKIP_PROMPT_HISTORY` is truthy the call is
/// a no-op.
pub fn add_to_history(entry: HistoryEntry) {
    if std::env::var("jet_SKIP_PROMPT_HISTORY")
...
/// Read `~/.jet/history.jsonl`, filter by `project`, and return up to
